...
class MainViewController: UIViewController, VirtualObjectSelectionViewControllerDelegate {
    ...
    @IBAction func chooseObject(_ button: UIButton) {
        // Prevent loading multiple objects
        if isLoadingObject { return }

        textManager.cancelScheduledMessage(forType: .contentPlacement)

        // Calculate appropriate size based on device
        let rowHeight = 60 // Match the row height we set in VirtualObjectSelectionViewController
        let width: CGFloat = UIDevice.current.userInterfaceIdiom == .pad ? 300 : min(300, view.bounds.width - 40)
        let height = CGFloat(rowHeight * VirtualObjectSelectionViewController.COUNT_OBJECTS)
        ...
        objectViewController.modalTransitionStyle = .crossDissolve

        if UIDevice.current.userInterfaceIdiom == .pad {
            objectViewController.modalPresentationStyle = .popover
            if let popoverController = objectViewController.popoverPresentationController {
...
//			self.loadVirtualObject()
	...
	@IBAction func takeSnapShot() {
		...
		// Temporarily hide the focus square
		focusSquare?.isHidden = true
		
		// Take a snapshot of the AR view
		let image = sceneView.snapshot()
		
		// Show focus square again
		...
		PHPhotoLibrary.requestAuthorization { status in
			if status == .authorized {
				PHPhotoLibrary.shared().performChanges({
					...
					if success {
						DispatchQueue.main.async {
							self.textManager.showMessage("Screenshot saved!")
						}
					} else if let error = error {
						DispatchQueue.main.async {
							self.textManager.showMessage("Error saving screenshot: \(error.localizedDescription)")
						}
					}
				}
			} else {
				DispatchQueue.main.async {
					self.textManager.showMessage("Photo library access denied")
				}
			}
		}
	}
	...
	@IBAction func showSettings(_ button: UIButton) {
		...
		// Use simple popover presentation for iPad
		if UIDevice.current.userInterfaceIdiom == .pad {
			...
			// Use simple modal presentation for iPhone
...
extension MainViewController: UIPopoverPresentationControllerDelegate {
    func adaptivePresentationStyle(for controller: UIPresentationController) -> UIModalPresentationStyle {
        return .none // Keeps it a popover on iPads
    }
